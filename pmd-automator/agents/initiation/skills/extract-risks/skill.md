...
description: Use this skill to create a complete Risk register from scratch for any project. Triggers when the user asks to "build a risk register", "create a risk log", "set up a risk register", "start a risk register for my project". Also use for requests like "I need to track project risks", "help me document risks", or "create a risk management document". Always use this skill when the user needs a foundational risk register artifact — Excel, Word, or PDF output.
---
...
* Project Statement of Work (SOW) document
* Stakeholder list or RACI matrix
* Purchase Order or project charter
...
* \[ ] **Project SOW** - Extract project scope, objectives, technical architecture, constraints, and assumptions

  * Expected filename pattern: `SOW\\\_\\\*.pdf` or `SOW\\\_\\\*.docx`
  * Location: `./project-artifacts/` directory
...
* \[ ] **Purchase Order / Project Charter** - Review budget, timeline, deliverables, and contractual terms
  ...
  * Expected filename pattern: `PO\\\_\\\*.docx` or `Project\\\_Charter\\\_\\\*.pdf`
  * Location: `./project-artifacts/` directory

**File Detection Procedure:**

* First check the `./project-artifacts/` working directory
...
```
\\\[ProjectName]\\\_Risk\\\_Register\\\_v\\\[VERSION].xlsx
```

Example: `SmartLife\\\_Mobile\\\_App\\\_Risk\\\_Register\\\_v1.0.xlsx`
